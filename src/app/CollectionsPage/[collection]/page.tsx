...
const CollectionPage = () => {
  ...
  // Convert slug back to proper name (e.g., "living" -> "Living")
  ...
  const collectionData = collectionsData.find(
    collection => collection.name.toLowerCase() === collectionSlug?.toLowerCase()
  );
  ...
  const collectionProducts = useMemo(() => {
    ...
    // Filter products that belong to this collection's categories
    return products.filter(product => {
      const productCategory = product.category.toLowerCase();
      return collectionData.categories.some(cat => 
        cat.name.toLowerCase().includes(productCategory) || 
        productCategory.includes(cat.name.toLowerCase()) ||
        cat.subcategories.some(sub => 
          sub.name.toLowerCase().includes(productCategory) || 
          productCategory.includes(sub.name.toLowerCase())
        )
      );
    });
  }, [collectionData]);
  ...
  const availableSubcategories = useMemo(() => {
    ...
    const category = collectionData?.categories.find(cat => 
      cat.name.toLowerCase() === selectedCategory.toLowerCase()
    );
    return category?.subcategories || [];
  }, [collectionData, selectedCategory]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedSubcategory !== 'all') {
      filtered = filtered.filter(product => 
        product.category.toLowerCase().includes(selectedSubcategory.toLowerCase()) ||
    ...
    return filtered;
  }, [collectionProducts, selectedCategory, selectedSubcategory, priceRange, sortBy]);
    ...
    return (
      <div className="min-h-screen bg-gray-50 pt-24 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">The collection "{collectionSlug}" doesn't exist.</p>
          <Link 
  ...
  const getPageTitle = () => {
    let title = `${collectionName} Collection`;
    if (selectedCategory !== 'all') {
  ...
  const getPageDescription = () => {
    if (selectedSubcategory !== 'all') {
      return `Explore our premium ${selectedSubcategory} collection for your ${collectionName.toLowerCase()} space`;
    }
    if (selectedCategory !== 'all') {
      return `Discover our ${selectedCategory} collection for your ${collectionName.toLowerCase()}`;
    }
    return `Discover our premium ${collectionName.toLowerCase()} furniture collection`;
  };
    ...
    <div className="min-h-screen bg-gray-50 pt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-600 mb-6">
          ...
          <span>/</span>
          <Link href="/collectionspage" className="hover:text-[#7A1315]">Collections</Link>
          <span>/</span>
          <Link href={`/collectionspage/${collectionSlug}`} className="hover:text-[#7A1315]">{collectionName}</Link>
                    ...
                    {availableSubcategories.map((subcategory) => (
                      <label key={subcategory.name} className="flex items-center">
                        <input
                          ...
                          checked={selectedSubcategory === subcategory.name.toLowerCase()}
                          onChange={(e) => setSelectedSubcategory(e.target.value)}
                          className="mr-3"
                        />
